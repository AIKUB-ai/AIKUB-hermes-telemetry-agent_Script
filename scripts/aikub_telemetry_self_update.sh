...
# Safe defaults: preserve .env, logs/, and state-like local files by moving the old
# non-git install aside during repair. Never prints token/env contents.
...
if [ -d "${AGENT_DIR}/.git" ]; then
  ...
  if [ -d "${OLD_DIR}" ]; then
    for item in .env logs state failed_chunks failed_log_entries.jsonl; do
      if [ -e "${OLD_DIR}/${item}" ] && [ ! -e "${AGENT_DIR}/${item}" ]; then
        cp -a "${OLD_DIR}/${item}" "${AGENT_DIR}/${item}"
        echo "preserved=${item}"
      fi
    done
  fi
